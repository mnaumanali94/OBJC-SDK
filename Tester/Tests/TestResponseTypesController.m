...
-(void) testGetHeadersAllowExtra {
    //Set up test expectations
    XCTestExpectation *expectation =[self expectationWithDescription:@"High Expectations"];

    // Set callback and perform API call
    [self.controller getHeadersWithCompletionBlock:  ^(BOOL success, HttpContext* context, NSError* error) { 
       // Test response code
        XCTAssertEqual(200, context.response.statusCode);
        ...
        [headers setObject:[NSNull null]  forKey:@"NauManAli"];
        
        [headers setObject:@"is also awesome"  forKey:@"WaseemHasAn"];
